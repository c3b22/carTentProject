delete FROM VehicleTransferRequestDocument;
delete FROM Insurance;
delete FROM InsuranceCompany;
delete FROM CarSaleContract;
delete FROM ReviewStaff;
delete FROM mydb.Order;
delete FROM PowerOfAttorney;
delete FROM BookingCar;
delete FROM Car;
delete FROM ColorCar;
delete FROM PreviousOwner;
delete FROM historyBranchStaff;
delete FROM Buyer;
delete FROM Staff;
delete FROM BranchTents;
delete FROM BrandTents;
delete FROM BrandOwner;
delete FROM AdminService;
...
(8, 'Hannah', 'Davis', 'F', '1991-06-30', 'R789012345678', '0975503344', 'Accountant', 48000, 3),
(9, 'Ian', 'Taylor', 'M', '1986-12-08', 'S654321098765', '0355588777', 'Salesperson', 32000, 3),
(258, 'John', 'Doe', 'M', '1990-05-15', '1234567890123', '0812345678', 'Manager', 50000, 258),
(259, 'Jane', 'Smith', 'F', '1992-08-20', '2345678901234', '0823456789', 'Salesperson', 35000, 259),
(260, 'Alice', 'Johnson', 'F', '1995-03-10', '3456789012345', '0834567890', 'Assistant', 30000, 260),
(261, 'Bob', 'Williams', 'M', '1988-11-25', '4567890123456', '0845678901', 'Technician', 40000, 258),
(262, 'Eva', 'Brown', 'F', '1991-07-05', '5678901234567', '0856789012', 'Clerk', 32000, 259),
(263, 'Michael', 'Garcia', 'M', '1989-09-18', '6789012345678', '0867890123', 'Janitor', 28000, 260),
...
select * from mydb.previousowner;
...
`idOrder`)
VALUES
(1, '2023-03-01', 4, 'Excellent service!', 2, 4145,1),
(2, '2023-03-02', 4, 'Good job overall.', 2,1456,2),
(258, '2021-01-01', '5', 'Excellent service', 258, 258, 16),
(259, '2021-02-01', '4', 'Good communication', 259, 259, 17),
(260, '2021-03-01', '3', 'Average assistance', 260, 260, 18),
(261, '2021-04-01', '2', 'Needs improvement', 261, 261, 19),
(262, '2021-05-01', '1', 'Poor service', 262, 262, 20),
(263, '2021-06-01', '0', 'Terrible experience', 263, 263, 21);

select * from `mydb`.`carsalecontract`;
...
select * from `mydb`.`vehicletransferrequestdocument`;
...
-- เพิ่มข้อมูลใน historyBranchStaff
...
select * from mydb.historyBranchStaff;
...
select * from mydb.order;
  ...
  -- (518, '/vtrd/vtrd_1.pdf', '2023-08-15', 6),
 -- ( 519, '/vtrd/vtrd_2.pdf', '2023-08-30', 7),
 -- ( 520, '/vtrd/vtrd_3.pdf', '2023-09-15', 8),
 -- (521, '/vtrd/vtrd_4.pdf', '2023-09-30', 9),
-- (522, '/vtrd/vtrd_4.pdf', '2023-09-30', 10);